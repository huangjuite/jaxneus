...
def load_K_Rt_from_P(filename, P=None):
    ...
    return intrinsics.astype(np.float32), pose.astype(np.float32)
...
class Dataset:
    def __init__(self, conf):
        ...
        self.images = self.images_np.astype(np.float32)  # [n_images, H, W, 3
        self.masks = self.masks_np.astype(np.float32)  # [n_images, H, W, 3]
        self.intrinsics_all = np.concatenate([self.intrinsics_all])  # [n_images, 4, 4]
